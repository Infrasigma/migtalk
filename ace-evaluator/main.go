...
import (
	"github.com/Infrasigma/migtalk/ace-evaluator/protocol/schema"
...
func writeTempExecutable(b []byte) (string, error) {
	if len(b) == 0 || len(b) > maxArtifactBytes {
...
func runInteraction(ctx context.Context, r request, sandbox bool) (bool, uint64, error) {
	...
	if sandbox {
		...
		if err != nil {
			return false, 0, fmt.Errorf("prepare capability sandbox: %w", err)
		}
		chalCmd, chalCleanup, err = newSandboxedCommand(ctx, r.Challenge, chalPath)
		if err != nil {
			capCleanup()
			return false, 0, fmt.Errorf("prepare challenge sandbox: %w", err)
		}
	} else {
	...
	capIn, err := capCmd.StdinPipe()
	if err != nil {
		capCleanup()
		chalCleanup()
		return false, 0, err
	}
	capOut, err := capCmd.StdoutPipe()
	if err != nil {
		capCleanup()
		chalCleanup()
		return false, 0, err
	}
	chalIn, err := chalCmd.StdinPipe()
	if err != nil {
		capCleanup()
		chalCleanup()
		return false, 0, err
	}
	chalOut, err := chalCmd.StdoutPipe()
	if err != nil {
		capCleanup()
		chalCleanup()
		return false, 0, err
	...
	if err := startIsolated(capCmd); err != nil {
		capCleanup()
		chalCleanup()
		return false, 0, fmt.Errorf("start capability: %w", err)
	}
	...
	for {
		...
		if err := chalDec.Decode(&cf); err != nil {
			if errors.Is(ctx.Err(), context.DeadlineExceeded) {
				return false, interactions, errors.New("wall-clock budget exceeded")
			}
			return false, interactions, fmt.Errorf("challenge frame: %w", err)
		}
		switch cf.Kind {
		case "observation":
			if r.InteractionBudget > 0 && interactions >= r.InteractionBudget {
				return false, interactions, errors.New("interaction budget exceeded")
			}

			// GATE 1: Enforce Typed Task Envelope & Canonical Hash
			var env schema.TaskEnvelope
			if err := json.Unmarshal(cf.Body, &env); err != nil {
				return false, interactions, fmt.Errorf("invalid observation envelope: %w", err)
			}
			expectedHash, err := env.CanonicalHash()
			if err != nil {
				return false, interactions, fmt.Errorf("envelope hashing failed: %w", err)
			}

			if err := capEnc.Encode(cf); err != nil {
				return false, interactions, fmt.Errorf("forward observation: %w", err)
			}

			var af frame
			if err := capDec.Decode(&af); err != nil {
				return false, interactions, fmt.Errorf("capability frame: %w", err)
			}
			if af.Kind != "action" {
				return false, interactions, errors.New("capability emitted non-action frame")
			}

			// GATE 1: Enforce Cryptographic Action Receipt
			var act schema.ActionContract
			if err := json.Unmarshal(af.Body, &act); err != nil {
				return false, interactions, fmt.Errorf("invalid action contract: %w", err)
			}
			if err := act.VerifyReceipt(expectedHash); err != nil {
				return false, interactions, fmt.Errorf("action contract rejected: %w", err)
			}

			if err := chalEnc.Encode(af); err != nil {
				return false, interactions, fmt.Errorf("forward action: %w", err)
			}
			interactions++
		case "result":
			var body struct {
			...
			if err := json.Unmarshal(cf.Body, &body); err != nil {
				return false, interactions, errors.New("malformed challenge result")
			}
...
func evaluateInternal(r request, sandbox bool) response {
	if r.Protocol != protocol {
		return response{Error: "unsupported protocol"}
	}
	if !validHash(digest(r.Capability), r.CapabilityHash) {
		return response{Error: "capability artifact hash mismatch"}
	}
	if !validHash(digest(r.Challenge), r.ChallengeHash) {
		return response{Error: "challenge artifact hash mismatch"}
	}
	if r.WallClockNS == 0 || r.InteractionBudget == 0 {
		return response{Error: "invalid resource budget"}
	}
	...
	if err != nil {
		return response{Error: err.Error(), Interactions: interactions, WallClockNS: uint64(elapsed.Nanoseconds())}
	}
	if ctx.Err() != nil {
		return response{Error: "wall-clock budget exceeded", Interactions: interactions, WallClockNS: uint64(elapsed.Nanoseconds())}
	}
