...
check_slice_limits() {
  local root="$1"
  [[ -d "$root" ]] || fail "cgroup directory does not exist: $root"
  ...
  finite_first_field "$cpu" || fail "CPU is unlimited at enforcing cgroup: $root"
  finite_first_field "$memory" || fail "memory is unlimited at enforcing cgroup: $root"
  finite_first_field "$pids" || fail "pids are unlimited at enforcing cgroup: $root"
  [[ "$swap" == 0 ]] || fail "swap is not disabled at enforcing cgroup $root: $swap"
}

runner_slice_path() {
  local runner_path="$1"
  [[ "$runner_path" == /* && "$runner_path" != *..* ]] || fail "invalid absolute runner cgroup path: $runner_path"
  local slice_path="${runner_path%/*}"
  [[ "${slice_path##*/}" == "$ACE_SLICE" ]] || fail "runner is not a direct child of ${ACE_SLICE}: $runner_path"
  printf '%s\n' "$slice_path"
}
...
check_workload_boundary() {
  ...
  [[ "$cpu" == "$ACE_CPU_MAX" ]] || fail "unexpected workload cpu.max: $cpu"
  [[ "$memory" == "$ACE_MEMORY_MAX" ]] || fail "unexpected workload memory.max: $memory"
