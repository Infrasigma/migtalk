...
# The evaluator's production path relies on unprivileged user namespaces to
# create user+mount+net+IPC+PID namespaces atomically. Test the real kernel;
# do not substitute a mock or a capability-file check.
unshare --user --map-root-user --mount --net --ipc --pid --fork true \
  || fail "kernel/container policy rejected required namespaces"
